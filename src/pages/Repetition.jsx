...
export default function Repetition() {
  ...
  const findBestButton = (selected) => {
    ...
    const vocalTutti = activeSong.audioButtons.find((b) => Array.isArray(b.pupitres) && b.pupitres.length === 0 && !isInstrumental(b))
        ...
        <div className="px-4 pt-2 pb-1 flex flex-col gap-1.5">
          {activeSong.notes?.trim() && (
            ...
            >
              ...
              <p className="text-xs text-amber-800 dark:text-amber-200 line-clamp-2 leading-relaxed" dangerouslySetInnerHTML={{ __html: activeSong.notes }} />
            </button>
          )}
